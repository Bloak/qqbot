    ...
    bot.on("message", (data)=>{
        ...
        if (!(globalBlacklist.includes(data.user_id))){
            ...
            if(data.user_id===3097847906){
                ...
                /*else if (data.message==='一键退群'){
                    var white_list = [105215419,154396709,170348681,239313171,586728511,701548657,716787742,780850390,912835656];
                    for (var [key,value] of bot.gl) {
                        if (!(white_list.includes(key))){
                            bot.sendGroupMsg(key, "小助改为私用，不再接受任何加好友/加群邀请。如有疑问请联系管理员。");
                            bot.setGroupLeave(key);
                        }
                    }
                    var friend_white_list = [3097847906,206433773];
                    for (var [key,value] of bot.fl) {
                        if (!(friend_white_list.includes(key))) {
                            bot.deleteFriend(key);
                        }
                    }
                }*/
                /*else if (data.message==='群发'){
                    for (var [gid, group_info] of bot.gml) {
                        for (var [qid, member_info] of group_info){
                            bot.sendPrivateMsg(qid, '请无视这条消息');
                        }
                    }
                }
                else if (data.message.slice(0,3)==='单发 '){
                    var qid = parseInt(data.message.slice(3));
                    bot.sendPrivateMsg(qid, '请无视这条消息');
                }*/
            ...
            if (true){//(data.message_type==='private'||[239313171,863689056,571354212,736375227,701548657,954212429].includes(data.group_id)){
                ...
                /*else if (data.message==='两个间谍'){
                    ...
                    var url = "https://163.lpddr5.cn/search?keywords="+encodeURI(name);
                    try{
                        fetch(url)
                            .then(res => res.json())
                            .catch((err) => {
                ...
                // jrcp
                else if (data.message_type==="group" && data.message === "jrcp") {
                ...
                //平版弹幕对战
                /*else if (data.message==='开始平版弹幕对战'){
                    var rawData = fs.readFileSync('danmaku.json');
                    var games = JSON.parse(rawData);

                    var self_id = data.user_id.toString();
                    if (self_id in games){
                        reply = '你已经加入了游戏！';
                    }
                    else {
                        ...
                        if (games.waiting===null){
                            games.waiting = self_id;
                        }
                        else {
                            var opponent_id = games.waiting;
                            games[self_id].opponent_id = opponent_id;
                            games[opponent_id].opponent_id = self_id;
                            games.waiting = null;
                        }
                        var newData = JSON.stringify(games);
                        fs.writeFileSync('danmaku.json',newData);
                    }
                }
                else if (data.message==='结束平版弹幕对战'){
                    var rawData = fs.readFileSync('danmaku.json');
                    var games = JSON.parse(rawData);

                    var self_id = data.user_id.toString();
                    if (self_id in games){
                        if (games.waiting===self_id) {
                            games.waiting = null;
                            delete games[self_id];
                            reply = '已取消匹配';
                        }
                        else {
                            var opponent_id = games[self_id].opponent_id;
                            delete games[self_id];
                            delete games[opponent_id];
                            reply = '已结束';
                            bot.sendPrivateMsg(parseInt(opponent_id), '你的对手提前离开了游戏');
                        }
                        var newData = JSON.stringify(games);
                        fs.writeFileSync('danmaku.json',newData);
                    }
            ...
            /*else {
                if (data.message==='帮助'){
                    reply = '本群开放功能：卡牌查询；卡牌检索；玩家简介；卡组查询；上传卡组；圣经咏唱。输入"帮助 卡牌查询"等查看具体指令。私聊"帮助"查看更多功能。';
                }
            }
            //通用功能
            if (data.message==='帮助 卡牌查询'){
                reply = '卡牌查询 卡牌名称。部分卡牌有玩家评价。发送“随机评价”可查看随机一个评价';
            }
            else if (data.message==='帮助 卡牌检索'){
                reply = '卡牌检索 名称=a#阵营!b#品质=c#类别=d#系列=e#能耗<f#力量>g#牌面描述{h\n"#"表示且，"="表示相同，"!"表示不同，"<"表示小于，">"表示大于，"{"表示包括';
            }
            else if (data.message==='帮助 玩家简介'){
                reply = '简介 玩家名';
            }
            else if (data.message==='帮助 卡组查询'){
                reply = '卡组查询 卡组名。发送“随机卡组”可查看随机一个卡组';
            }
            else if (data.message==='帮助 上传卡组'){
                reply = '上传 卡组名 代码或图片。注意格式：两个空格，不要加回车';
            }
            else if (data.message==='帮助 圣经咏唱'){
                reply = '圣经咏唱 玩家名'
            }
            else if (data.message.slice(0,5)==='卡牌查询 '){
                reply = '该功能暂时关闭';
                /*var rawData = fs.readFileSync('2047_cards.json');
                var cards = JSON.parse(rawData);
                if (data.message.slice(5) in cards){
                    var card = cards[data.message.slice(5)];
                    if (card.类别 === '生物'){
                        reply = `${card.名称}（${card.系列}）\n${card.阵营}${card.品质}\n${card.能耗}/${card.力量}\n${card.牌面描述}`;
                        if ('评价' in card && card.评价.length > 0){
                            reply += `\n玩家评价：${card.评价[random(0,card.评价.length)]}`;
                        }
                    }
                    else {
                        reply = `${card.名称}（${card.系列}）\n${card.阵营}${card.品质}\n${card.能耗}费\n${card.牌面描述}`;
            ...
            else if (data.message.slice(0,5)==='卡牌检索 '){
                reply = '该功能暂时关闭';
                /*var cardNames = search(data.message.slice(5));
                reply = ''
            ...
            else if (data.message.slice(0,3)==='简介 '){
                var rawData = fs.readFileSync('player_profile.json');
                var playerProfiles = JSON.parse(rawData);
                if (data.message.slice(3) in playerProfiles){
                    if (data.message==='简介 官方指定复读机'){
                        reply = '简介 官方指定复读机';
                    }
                    else {
                        reply = data.message.slice(3) + ': ' + playerProfiles[data.message.slice(3)];
                    }
                }
                else {
                    reply = '这名玩家还没有简介哦';
            ...
            else if (data.message.slice(0,5)==='卡组查询 '){
                var rawData = fs.readFileSync('deck.json');
                var decks = JSON.parse(rawData);
                if (data.message.slice(5) in decks){
                    reply = data.message.slice(5) + ': ' + decks[data.message.slice(5)];
                }
                else {
                    reply = '未记录该卡组';
                }
            }
            else if (data.message==='随机卡组'){
                var rawData = fs.readFileSync('deck.json');
                var decks = JSON.parse(rawData);
                var deckName = Object.keys(decks)[random(0,Object.keys(decks).length)];
                reply = deckName + ': ' + decks[deckName];
            }
            else if (data.message==='随机评价'){
                var rawData = fs.readFileSync('card_comments.json');
                var comments = JSON.parse(rawData);
                while (true){
                    var card = Object.keys(comments)[random(0,Object.keys(comments).length)];
                    if (card.length>0){
                        var comment = comments[card][random(0,comments[card].length)];
                        break;
                    }
                }
                reply = comment;
            }
            else if (data.message.slice(0,3)==='上传 '){
                var tempList = data.message.slice(3).split(' ');
                var deckName = tempList[0];
                var code = tempList[1];
                var rawData = fs.readFileSync('deck.json');
                var decks = JSON.parse(rawData);
                if (deckName in decks){
                    ...
                    fs.writeFileSync('deck.json',newData);
                    reply = '上传成功';
                    bot.sendPrivateMsg(3097847906, `${data.user_id}上传了卡组${deckName}：${code}`);
                }
            }
            else if (data.message.slice(0,5)==='圣经咏唱 '){
                var name = data.message.slice(5);
                var rawData = fs.readFileSync('sacred_words.json');
                var words = JSON.parse(rawData);
                if (name in words){
                    reply = words[name][random(0,words[name].length)];
                }
                else {
                    reply = '该玩家还没有语录哦';
                }
            ...
            // list message test
            if (data.user_id===3097847906 && data.message==="draw circle") {
